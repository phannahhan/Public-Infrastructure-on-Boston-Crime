...
suffolk_map <- suffolk %>%
  ...
  labs(caption = "Source: American Community Survey 2014-2018") +
  theme_void()
...
suffolk_crime_map <- suffolk_map +
  geom_point(crime, mapping = aes(Long, Lat), color = "red", alpha = 0.01) +
  labs(title = "Crime Map of Suffolk County, MA",
       caption = "Source: ANALYZE BOSTON CRIME INCIDENT REPORTS (AUGUST 2015 - TO DATE)") +
  theme_void()

suffolk_tree_map <- suffolk_map +
  geom_point(trees, mapping = aes(X, Y), color = "dark green", alpha = 0.01) +
  labs(title = "Tree Map of Suffolk County, MA",
       caption = "Public Tree Data 2011") +
  theme_void()

suffolk_hubway_map <- suffolk_map +
  geom_point(hubway, mapping = aes(X, Y), color = "red") +
  theme_void()

suffolk_bluebike_map <- suffolk_map +
  geom_point(bluebikes, mapping = aes(latitude, longitude), color = "blue") +
  theme_void()

suffolk_streetlight_map <- suffolk_map +
  geom_point(streetlights, mapping = aes(lat, long), color = "yellow") +
  theme_void()
